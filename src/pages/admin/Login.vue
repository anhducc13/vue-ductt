<template>
  <div class="login-container">
    <div class="content">
      <div class="top">
        <div class="header">
          <img alt="logo" class="logo" src="https://iczer.gitee.io/vue-antd-pro/static/img/vue-antd-logo.png" />
          ...
          <a-form @submit.prevent="handleSubmit(onSubmit)">
            <a-tabs size="large" :tabBarStyle="{textAlign: 'center'}" style="padding: 0 2px;">
              <a-tab-pane tab="Sử dụng TK-MK" key="1">
                <validation-provider name="Tên đăng nhập" rules="required|minLength:6" v-slot="{ errors }">
                  <a-form-item :help="errors[0]" :validate-status="errors[0] ? 'error': ''">
                    <a-input ref="username" size="large" placeholder="Nhập tên đăng nhập" v-model="formData.username" >
                ...
                <validation-provider name="Mật khẩu" rules="required|minLength:8" v-slot="{ errors }">
                  <a-form-item :help="errors[0]" :validate-status="errors[0] ? 'error': ''">
                    <a-input type="password" ref="password" size="large" placeholder="Nhập mật khẩu" v-model="formData.password">
            ...
            <a-form-item>
              <a-button :loading="loading" style="width: 100%;margin-top: 24px" size="large" htmlType="submit" type="primary">Đăng nhập</a-button>
            </a-form-item>
            <div>
              Phương thức khác
...
export default {
  ...
  methods: {
    onSubmit () {
      this.loading = true
      this.$store.dispatch('user/login', {...this.formData, loginAdmin: true})
        .then(() => {
